...
extratxhash=$(echo "$output" | grep -w TxOutDatumNone | rev | sort -r | rev | head -n 1 | cut -d ' ' -f1)
extratxix=$(echo "$output" | grep -w TxOutDatumNone | rev | sort -r | rev |head -n 1 | tr -s ' ' | cut -d ' ' -f2)
